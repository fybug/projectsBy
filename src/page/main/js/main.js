 ...
 * @return {string} 获取的值
 */
window.getQueryString = (key, defaval = undefined) => {
...
window.addEventListener("load", () => {
    // 当前请求的位置
    let index = Number(getQueryString("p", '0'));
    let tag = getQueryString("tag");
    const root = document.querySelector("#contlist > div nav");

    if (tag !== undefined) {
        document.querySelectorAll(".tags").forEach(v => v.remove());

        if (tag === '') showtag(root);
        else plushtagcont(root);

    } else getCont(index, root);
});